...
static unsigned int normalized_sysctl_sched_wakeup_granularity = 10000000UL;
...
{
	long delta_sum,
		delta_avg = gcfs_rq->avg.runnable_avg - se->avg.runnable_avg;
	u32 new_sum, divider;
...
static inline int update_cfs_rq_load_avg(u64 now, struct cfs_rq *cfs_rq)
{
	...
	if (cfs_rq->removed.nr) {
		...
		sa->load_sum = max_t(u32, sa->load_sum,
				     sa->load_avg * PELT_MIN_DIVIDER);
...
static inline bool sched_reduced_capacity(struct rq *rq,
					  struct sched_domain *sd)
{
	/*
...
static void update_idle_cpu_scan(struct lb_env *env, unsigned long sum_util)
{
	struct sched_domain_shared *sd_share;
	int llc_weight, pct;
	u64 x, y, tmp;
	...
	if (local->group_type == group_has_spare) {
		...
		if (env->sd->flags & SD_NUMA) {
			env->imbalance = adjust_numa_imbalance(
				env->imbalance, local->sum_nr_running + 1,
				env->sd->imb_numa_nr);
		}
...
{
	struct rq *this_rq = this_rq();
	enum cpu_idle_type idle = this_rq->idle_balance ? CPU_IDLE :
							  CPU_NOT_IDLE;
